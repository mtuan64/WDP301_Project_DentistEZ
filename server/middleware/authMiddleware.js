...
const authAdminOrStaffMiddleware = (req, res, next) => {
  const authHeader = req.headers.authorization;
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    return res.status(401).json({
      message: "Không tìm thấy token hoặc token không hợp lệ",
  ...
  jwt.verify(token, process.env.JWT_SECRET, function (err, decoded) {
    ...
    if (decoded.role === "admin" || decoded.role === "staff") {
      ...
      return res.status(403).json({
        message: "Bạn không có quyền truy cập (chỉ dành cho Admin hoặc Staff)",
        status: "ERROR",
      });
    }
  });
...
const authAdminOrStaffOrDentistMiddleware = (req, res, next) => {
  const authHeader = req.headers.authorization;
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    return res.status(401).json({
      message: "Không tìm thấy token hoặc token không hợp lệ",
  ...
  jwt.verify(token, process.env.JWT_SECRET, function (err, decoded) {
    ...
    if (
      ...
      return res.status(403).json({
        message:
          "Bạn không có quyền truy cập (chỉ dành cho Admin, Staff hoặc Dentist)",
        status: "ERROR",
      });
    }
  });
