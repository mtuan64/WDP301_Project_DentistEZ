...
const EditAppointmentByStaff = ({ visible, onCancel, appointment, onUpdate }) => {
  ...
  const fetchAvailableTimeslots = (date) => {
    ...
    const currentSlot = filteredSlots.find((slot) => slot._id === (appointment?.timeslot?._id || appointment?.timeslotId?._id));
    if (currentSlot && date === moment(appointment?.timeslot?.date || appointment?.timeslotId?.date).format('YYYY-MM-DD')) {
  ...
  useEffect(() => {
    if (visible && appointment) {
      const apptDate = (appointment?.timeslot?.date || appointment?.timeslotId?.date)
        ? moment(appointment.timeslot?.date || appointment.timeslotId?.date, 'YYYY-MM-DD')
        : null;
      ...
      form.setFieldsValue({
        ...
        timeslotId: appointment?.timeslot?._id || appointment?.timeslotId?._id,
      });
      const serviceId = appointment?.service?._id || appointment?.serviceId?._id;
      if (serviceId) {
        fetchServiceDetail(serviceId);
      } else {
        message.error('Không tìm thấy ID dịch vụ để tải chi tiết');
      }
    }
  ...
  const handleTimeSlotSelect = (slot) => {
    if (slot.isAvailable !== false || slot._id === (appointment?.timeslot?._id || appointment?.timeslotId?._id)) {
  ...
  return (
    <Modal
      title="Chỉnh sửa lịch hẹn tái khám"
      open={visible}
      onCancel={onCancel}
      footer={null}
    ...
    >
      <div style={styles.infoSection}>
        <div>
          ...
          {appointment?.service?.serviceName || appointment?.serviceId?.serviceName || 'N/A'}
        </div>
        <div>
          <span style={styles.infoLabel}>Bác sĩ:</span>
          {appointment?.doctorUser?.fullname || appointment?.doctorId?.userId?.fullname || 'N/A'}
        </div>
        <div>
              ...
              {timeslots.map((slot) => (
                <div
                  ...
                  style={{
                    ...
                    ...(slot.isAvailable === false && slot._id !== (appointment?.timeslot?._id || appointment?.timeslotId?._id)
...
export default EditAppointmentByStaff;